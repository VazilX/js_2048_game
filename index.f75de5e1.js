...
class Tile {
    ...
    findPlase() {
        ...
        for(let r = 0; r < Tile.FIELD_ROWS.length; r++){
            ...
            for(let c = 0; c < columns.length; c++)if (!columns[c].firstChild) freeFields.push({
                r,
                c
            });
        }
...
document.addEventListener("keydown", (ev)=>{
    ...
    for(let ii = 1; ii < COUNT_FILDS_CELL; ii++){
        ...
        if (ev.key === "ArrowUp") {
            ...
            if (intationMoveAll.length === 0) break;
            intationMoveAll.forEach((tile)=>{
                tile.tile.style.transform = `translateY(calc(-${ii * 100}% - ${ii * 10}px))`;
                tile.moved = true;
            });
        ...
        if (ev.key === "ArrowLeft") {
            for(let colamn = 1; colamn < COUNT_FILDS_CELL; colamn++){
                const intationMove = Tile.tiles.filter((tile)=>tile.coords.c === colamn);
                intationMove.forEach((tile)=>{
            ...
            if (intationMoveAll.length === 0) break;
            intationMoveAll.forEach((tile)=>{
                tile.tile.style.transform = `translateX(calc(-${ii * 100}% - ${ii * 10}px))`;
                tile.moved = true;
            });
            forMoveInHtml.push(...intationMoveAll);
        }
